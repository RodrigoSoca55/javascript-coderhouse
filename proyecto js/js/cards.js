...
function addToLocalStorage(id,quantity,price) {
  let stringCart = JSON.stringify({id,quantity,price})
  cart.push(stringCart)
...
function cards() {
  ...
  for (let i = 0; i < productos.length; i++) {
    ...
    <div class="container-img">
        ...
        <button  onclick='addToLocalStorage(${productos[i].id},5,${result})'> add to cart</button>`
  ...
  if (searchResult) {
    ...
    card.innerHTML = `
     <div class="container-img">
         ...
         <button  onclick="addToLocalStorage(${searchResult.id},5,${result})"> add to cart</button>`
       contenedor.appendChild(card)
